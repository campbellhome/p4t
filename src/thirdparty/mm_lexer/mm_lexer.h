...
# define LEXER_PTR_TO_UINT(x) ((lexer_size)(((char*)x)-(char*)0))
...
# define LEXER_PTR_TO_UINT(x) ((lexer_size)(x))
#endif
...
lexer_read_string(struct lexer *lexer, struct lexer_token *token, int quote)
{
    ...
    if (token->str) {
        // MDC:
        if (end) token->len = (lexer_size)(end - token->str) - 1;
        else token->len = (lexer_size)(lexer->current - token->str) - 1;
